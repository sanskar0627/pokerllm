...
export async function GET() {
  ...
  const wins = gameRecords.filter((g: { result: string | null }) => g.result === 'win').length
  const losses = gameRecords.filter((g: { result: string | null }) => g.result === 'loss').length
  const abandoned = gameRecords.filter((g: { result: string | null }) => g.result === 'abandoned').length
  ...
  return NextResponse.json({
    ...
    aiProfiles,
    gameRecords: gameRecords.map((g: { id: string; gameId: string; models: string[]; rounds: number; result: string | null; createdAt: Date }) => ({
      id: g.id,
      gameId: g.gameId,
      models: g.models,
      rounds: g.rounds,
